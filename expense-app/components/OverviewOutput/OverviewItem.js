...
function CategoryItem({
  ...
  const description = (txs, i) => {
    ...
    if (txs.amount) {
      amount = (
        <Text style={[styles.amount, styles.textBase]}>$ {txs.amount}</Text>
      );
    }
    if (txs?.type) {
      type =
        txs.type === EXPENSE ? (
          ...
          txs.type === INCOME && <Text style={[styles.textBase]}>incurred</Text>
        );
    }
    if (txs.category) {
      category = (
    ...
    return (
      <View onStartShouldSetResponder={() => true} key={txs.id}>
        ...
        {txs && txs.email && <Text style={styles.textBase}>{txs.email}</Text>}
        <Divider />
      </View>
    );
  };
